...
/** Perf dot-line (v3 "Skin A"): dots the stage colour shows through, 10px on-centre. `var(--bg)`
 * in the recipe = the dark stage the paper floats on, so the holes read as cut-through paper. */
const perfStyle: CSSProperties = {
  backgroundImage: `radial-gradient(circle, ${colors.bg} 40%, transparent 46%)`,
  backgroundSize: `${PERF_SIZE}px ${PERF_SIZE}px`,
  backgroundRepeat: 'repeat-x',
  backgroundPosition: 'center',
};

/** 14px die-cut notch in the stage colour, half in / half out of the card edge. */
...
export function TicketFrame({
  ...
  // Only claim a positioning context when something inside needs one (notches / overlay). A
  // hardcoded `relative` would silently outrank a caller's `absolute` on same specificity via
  // source order (the UnderCard trap — see BallotCard's UnderCard doc comment), so leave
  // `position` to the caller when there's nothing absolutely positioned within.
  ...
  return (
    <div
      ...
      className={`${positioned ? 'relative ' : ''}flex flex-col rounded-lg ${t.surface} ${className}`}
    >
      {notches ? (
        <>
          <span
            aria-hidden="true"
            className="absolute top-1/2 -left-[7px] -translate-y-1/2 rounded-full"
            style={notchStyle}
          />
          <span
            aria-hidden="true"
            className="absolute top-1/2 -right-[7px] -translate-y-1/2 rounded-full"
            style={notchStyle}
          />
        </>
      ) : null}
      ...
      {perfTop ? (
        <div aria-hidden="true" className="h-1.5 -translate-y-1" style={perfStyle} />
      ) : null}
