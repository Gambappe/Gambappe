/**
 * `POST /api/v1/rematch-requests/:id/decline` (design doc §8.4 step 0, §9.2, WS5-T5). Claimed;
 * target-only (enforced in `@/lib/nemesis/rematch`'s `respondToRematchRequest`). Replaces
 * `apps/web/lib/nemesis/mock-api.ts`'s `declineRematchRequest` mock (deleted by this task).
 */
...
export async function POST(
  ...
  return runRoute(async () => {
    ...
    const result = await respondToRematchRequest(getDb(), id, identity.profile.id, 'decline', now());
