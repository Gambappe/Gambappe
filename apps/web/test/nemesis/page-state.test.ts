...
describe('selectNemesisPageState', () => {
  it('is "assignment" whenever there is an active pairing, regardless of history', () => {
    expect(selectNemesisPageState({ pairing: PAIRING, historyEntries: [] }).kind).toBe(
      'assignment',
    );
    expect(
      selectNemesisPageState({ pairing: PAIRING, historyEntries: [historyEntry()] }).kind,
    ).toBe('assignment');
  });

  it('is "verdict" for the most recent history entry when there is no active pairing and it has a real outcome', () => {
    const entry = historyEntry({ outcome: 'loss' });
    const state = selectNemesisPageState({ pairing: null, historyEntries: [entry] });
  ...
  it('promotes only the FIRST (newest) history entry, never an older one', () => {
    ...
    const state = selectNemesisPageState({ pairing: null, historyEntries: [newest, older] });
  ...
  it('is "empty" when there is no active pairing and no history at all', () => {
    expect(selectNemesisPageState({ pairing: null, historyEntries: [] }).kind).toBe('empty');
  });
  ...
  it('is "empty" (not "verdict") when the most recent entry is cancelled — a cancelled week never gets a verdict card', () => {
    ...
    expect(selectNemesisPageState({ pairing: null, historyEntries: [entry] }).kind).toBe('empty');
  });
  ...
  it('falls back to "empty" if the most recent entry is cancelled even when older entries have real outcomes — no reaching past the newest', () => {
    ...
    expect(
      selectNemesisPageState({ pairing: null, historyEntries: [cancelled, olderWin] }).kind,
    ).toBe('empty');
  });
});
